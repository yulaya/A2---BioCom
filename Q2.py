...
def printMotifApearancesAndSubgraphs(motifsApearances,subgraphs,numMotifs,n):
  print(f"n={n}")
  for i,g in enumerate(subgraphs):
    print(f"#{i+1}") #print the "index" of the subgraph, and then the subgraph
    for u,v in g.edges():
      print(f"{u+1} {v+1}")
  ...
  for k in range (numMotifs):
      print(f"Motif #{k+1}:{motifCount[k]}")
...
def main():
  ...
  printMotifApearancesAndSubgraphs(mA,sG,nM,n)
